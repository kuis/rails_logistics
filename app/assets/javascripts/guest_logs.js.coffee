...
$ ->
  ...
  $('.summernote').summernote({
    ...
    onkeyup: (e) ->
      ...
      if isAlarmMsg
        $('#btn-new-log').html("<i class='ico-checkmark'/> <span class='hidden-xs'>Save Alarm</span>")
      else
  ...
  $('#btn-new-log').on 'click', (e) ->
    if $('#btn-new-log').hasClass('btn-success')
      ...
      if $(this).text() == ' Save Alarm'
        ...
        $('#alarmConfirmationDialog #message').text(log.substr(0, results[0].index - 1))
  ...
  $('#alarmConfirmationDialog .confirm').on 'click', (e) ->
    $.ajax(
      ...
      dataType: 'json'
      data:
        body: $('#alarmConfirmationDialog #message').text()
