$ ->
  ...
  load_and_build_equipments_table = (filter_type = 'remaining')->
    ...
    .done (data) ->
      ...
      $('#main .indicator').removeClass('hide')
      $.ajax(Routes.maintenance_records_path(status: 'in_progress', type: 'Maintenance::Equipment'), {type: 'GET', dataType: 'JSON'}).done (records) ->
        equipmentsTable.api().clear().draw()
      ...
      .complete (res) -> $('#main .indicator').addClass('hide')
    .complete (res) -> $('#main .indicator').addClass('hide')
