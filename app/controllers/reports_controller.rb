class ReportsController < ApplicationController
      ...
      if current_cycle.nil?
        ...
        if params[:report_type] == 'cyclely'
          unless params[:property_switched].present? # if property_switched, ignore cycle param and return current cycle data
            cycle = Maintenance::Cycle.where(ordinality_number: params[:cycle])
                                      .by_year(Time.current.year)
                                      .by_cycle_type(:room)
                                      .first
          end
          cycle ||= current_cycle
          ...
          respond_to do |format|
            ...
            format.csv  { }
            format.pdf do
              title = format("Guest Room PM Analysis - #{cycle.frequency_months == 3 ? 'Quarter' : 'Cycle'} #{params[:cycle]}")
              render pdf: title,
          ...
          cycles = Maintenance::Cycle.by_year(Time.current.year).by_cycle_type(:room)
          missing_rooms = 0
          total_rooms = 0
      ...
      if current_cycle(:public_area).nil?
        ...
        if params[:report_type] == 'cyclely'
          ...
          respond_to do |format|
            ...
            format.csv  { }
            format.pdf do
              title = "Public Area PM Analysis - #{cycle.frequency_months == 3 ? 'Quarter' : 'Cycle'} #{params[:cycle]}"
              render pdf: title,
          ...
          cycles = Maintenance::Cycle.by_year(Time.current.year).by_cycle_type(:public_area).last(6)
          missing_areas = 0
          total_areas = 0
          ...
          respond_to do |format|
            ...
            format.csv  { }
            format.pdf do
              title = format("Guest PM Room Analysis Yearly Report")
              render pdf: title,
  ...
  def set_property_info(cycle_type)
    ...
    response.headers['min_cycle'] = Maintenance::Cycle.by_year(Date.today.year).by_cycle_type(cycle_type).minimum(:ordinality_number).to_s
    response.headers['max_cycle'] = current_cycle(cycle_type).ordinality_number.to_s
    ...
    response.headers['current_cycle'] = current_cycle(cycle_type).ordinality_number.to_s
    response.headers['frequency_months'] = current_cycle(cycle_type).frequency_months.to_s
    response.headers['cycle_term'] = current_cycle(cycle_type).frequency_months == 3 ? 'Quarter' : 'Cycle'
  end
end
